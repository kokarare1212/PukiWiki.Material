...
function plugin_search_search_form($s_word = '', $type = '', $bases = array())
{
	...
	if (!empty($bases)) {
		...
		foreach($bases as $base) {
			...
			$base_msg  .=<<<EOD
 <div>
  <label><input type="radio" name="base" value="$s_base" $check> <span>$base_label</span></label>
 </div>
EOD;
			$check = '';
		}
		$base_msg .=<<<EOD
  <label><input type="radio" name="base" value=""> <span>$_search_all</span></label>
EOD;
...
<form action="$script?cmd=search" method="post">
 <div>
  ...
  <label><input type="radio" name="type" value="AND" $and_check> <span>$_btn_and</span></label>
  <label><input type="radio" name="type" value="OR" $or_check> <span>$_btn_or</span></label>
  &nbsp;<button class="btn waves-effect waves-light" type="submit">$_btn_search</button>
 </div>
$base_option
</form>
EOD;
